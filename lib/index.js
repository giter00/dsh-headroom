...
const MARKER_STYLES = new Set(['full', 'compact'])
...
function markerStyleOf(value) {
  const resolved = value ?? 'full'
  if (!MARKER_STYLES.has(resolved)) {
    throw new Error(`dsh-headroom: markerStyle must be one of ${[...MARKER_STYLES].join(', ')}, got ${String(resolved)}`)
  }
  return resolved
}
...
export function normalizeConfig(raw = {}) {
  ...
  return Object.freeze({
    ...
    noFoldForTools: raw.noFoldForTools === undefined
      ? [...DEFAULT_LIMITS.noFoldForTools]
      : stringArray(raw.noFoldForTools, 'noFoldForTools'),
    noFoldForPatterns: raw.noFoldForPatterns === undefined
      ? [...DEFAULT_LIMITS.noFoldForPatterns]
      : stringArray(raw.noFoldForPatterns, 'noFoldForPatterns'),
    markerStyle: markerStyleOf(raw.markerStyle),
...
const PATH_ARG_KEYS = new Set([
  ...
  'src', 'target', 'directory', 'dir', 'root', 'cwd', 'folder'
])

function collectPathCandidates(value, seen = new Set(), out = []) {
  if (typeof value === 'string') {
    // Bare path-like strings (usually from array args). Keep globs like
    // "*.js" out so grep `include` patterns do not disable compression.
    if (/[\\/]/.test(value) || (/\.[A-Za-z0-9]{1,8}$/.test(value) && !value.includes('*'))) {
      out.push(value)
    }
    return out
  }
  ...
  if (Array.isArray(value)) {
    for (const item of value) collectPathCandidates(item, seen, out)
    return out
  }
  for (const [key, child] of Object.entries(value)) {
    if (typeof child === 'string') {
      if (PATH_ARG_KEYS.has(key) || /[\\/]/.test(child)) {
        out.push(child)
      }
    } else {
      collectPathCandidates(child, seen, out)
    }
  }
  return out
}

function pathPatternMatches(config, exec) {
  if (config.noFoldForPatterns.length === 0) return false
  const candidates = collectPathCandidates(exec.args)
  return candidates.some((candidate) => isGlobMatch(candidate.replace(/\\/g, '/'), config.noFoldForPatterns))
}

/**
 * File-tool safety: never let the display layer change bytes that read/edit or
 * str_replace_editor depend on. `noFoldForTools` skips exact tool names/globs;
 * `noFoldForPatterns` additionally protects paths matching source/config globs
 * (e.g. *.js) even when the tool name is generic (pwsh/node).
 */
function shouldSkipCompression(config, exec) {
  if (config.noFoldForTools.length > 0 && isGlobMatch(exec.name, config.noFoldForTools)) return true
  if (pathPatternMatches(config, exec)) return true
  return false
}
...
function makePostExecuteListener(config, ccrStore, logger) {
  ...
  return async function postExecuteListener(exec, result, next) {
    ...
    if (shouldSkipCompression(config, exec)) return decision
