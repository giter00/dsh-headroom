 ...
 *
 *   ContentRouter → SmartCrusher (JSON) | search-fold | log-fold | text-head/tail
 *
 * The full Headroom ML Kompress model is intentionally NOT bundled here; the
 * fallback for prose is reversible head/tail truncation: every lossy
 * compression emits a CCR marker so the model can call `headroom_retrieve` to
 * recover the exact original. The caller owns the CCR store and passes the
 * retrieval id in.
 */
...
function compressSearchText(text, limits) {
  ...
  for (const [file, matches] of groups) {
    ...
    for (const match of kept) {
      ...
      const rest = truncateMiddle(match.rest, limits.maxCellChars)
      out.push(`${match.lineNo}${colText}: ${rest}`)
    }
...
export function detectContentType(text) {
  ...
  if (markerCount >= 3 || rawLines.some((line) => LOG_ERROR_RE.test(line)) || hasRepeatedLines(lines)) return 'log'

  if (CODE_HINTS.some((pattern) => pattern.test(text))) return 'code'
 ...
 */
export function markerFor(retrievalId, strategy, originalChars, compressedChars) {
  return `\n\n[headroom: ${strategy} ${originalChars}→${compressedChars} chars; retrieve full original with headroom_retrieve(id="${retrievalId}")]`
}
...
export function compressTextBlock(text, options = {}) {
  ...
  if (kind === 'json') {
    ...
    candidate = compressPlainText(text, limits)
  ...
  const marker = withMarker
    ? markerFor(options.retrievalId, strategy, originalChars, candidate.length)
    : ''
